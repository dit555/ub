...
typedef enum ping_states { PInit, PI_On, PI_Off} ping_states;
typedef enum detect_eq_states { DEQInit, DEQ_T, DEQ_F} detect_eq_states;
typedef enum detect_max_amp_states { DMAInit, DMA_Wait, DMA_Ch} detect_max_amp_states;
typedef enum detect_zc_states { DZCInit, DZC_On, DZC_Off} detect_zc_states;
typedef enum transmit_states {TInit,                                        } transmit_states;

/* shared variables --- fill in as needed */
unsigned char eq_running = 0;
